...
// Допустим, у тебя есть хук для мутации статуса:
...
export const OrderStatusSelect = ({ 
  ...
  // Заглушка для хука мутации (React Query)
  // const { mutate: updateStatus, isPending } = useUpdateOrderItemStatus();
  const [isPending, setIsPending] = useState(false); // Временный стейт для UI
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Имитация запроса (здесь будет вызов mutate(newStatus))
    setIsPending(true);
  ...
  return (
    <div className="relative flex items-center">
      <select
        ...
        disabled={isPending}
        className={`appearance-none pl-4 pr-8 py-2 rounded-lg border text-sm font-medium outline-none cursor-pointer transition-colors disabled:opacity-50 disabled:cursor-wait ${getStatusColor(status)}`}
      >
      ...
      {isPending ? (
        <Loader2 className="absolute right-2.5 w-4 h-4 animate-spin text-current opacity-70 pointer-events-none" />
         ...
         <svg className="absolute right-2.5 w-4 h-4 text-current opacity-70 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
      )}
    </div>
  );
};