...
export default function ClientChatPage({
  ...
  // === СТЕЙТЫ ДЛЯ ВЫДЕЛЕНИЯ СООБЩЕНИЙ ===
  ...
  const {
    ...
    sendTypingStatus,
    isCompanionTyping,
    deleteMessages, // Эта функция теперь должна принимать (ids, forEveryone)
  ...
  // Скролл вниз (отключаем при выделении, чтобы не сбивать фокус)
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-120px)] min-h-[500px] flex flex-col bg-[#1A1F2E]/80 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-[#0f121b]">
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              ...
              {/* Row: checkbox + bubble */}
              <div
                className={`relative z-10 flex items-end gap-2 min-w-0 max-w-[85%] sm:max-w-[70%] lg:max-w-[60%] ${isMine ? "flex-row-reverse" : "flex-row"}`}
              >
                {/* Checkbox */}
                {isSelectionMode && (
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mb-1 transition-all ${
                      isSelected
                        ? "bg-[#8B7FFF] border-[#8B7FFF] scale-110"
                        : "border-[#A0AEC0]/60 bg-transparent"
                    }`}
                  >
        ...
        rounded-2xl text-sm sm:text-base shadow-sm
        transition-transform
        ${isSelectionMode && isSelected ? "scale-[0.98]" : ""}
        ${
          isMine
      ...
      `}
                >
                  ...
                  {/* Time + read status */}
                  <div
                    className={`flex items-center gap-1 mt-1 text-[10px] leading-none ${
                      isMine
                        ? "justify-end text-white/70"
                        : "justify-start text-[#A0AEC0]"
                    }`}
                  >
                    <span>{time}</span>
      ...
      <div className="p-4 sm:p-6 bg-[#1A1F2E] border-t border-white/5 relative overflow-hidden">
        <AnimatePresence mode="wait">
          {isSelectionMode ? (
            // Панель массового удаления с двумя кнопками
            <motion.div
              key="delete-panel"
