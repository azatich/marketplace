  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-120px)] min-h-[500px] flex flex-col bg-[#1A1F2E]/80 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="flex items-center gap-4 p-4 sm:p-6 border-b border-white/5 bg-white/2 transition-colors">
        {isSelectionMode ? (
          <>
            <button onClick={cancelSelection} className="p-2 -ml-2 rounded-xl text-white hover:bg-white/10 transition-colors">
              <X className="w-5 h-5" />
            </button>
            <div className="flex-1 font-semibold text-white">Выбрано: {selectedMessages.size}</div>
          </>
        ) : (
          <>
            <button onClick={() => router.back()} className="p-2 -ml-2 rounded-xl text-[#A0AEC0] hover:text-white hover:bg-white/5 transition-colors">
            ...
            <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 border border-[#8B7FFF]/30 bg-[#8B7FFF]/10 flex items-center justify-center">
              {displayInfo?.avatar_url ? (
                <img src={displayInfo.avatar_url} alt="Store" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="flex-1">
              <h2 className="font-bold text-white leading-tight">{displayInfo?.full_name || "Загрузка..."}</h2>
              <div className="text-xs mt-0.5 h-4 flex items-center">
                {isCompanionTyping ? (
                  <div className="flex items-center gap-1">
                    <span className="text-[#8B7FFF] font-medium italic">печатает</span>
                    <div className="flex gap-1">
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1 }} className="w-1 h-1 bg-[#8B7FFF] rounded-full" />
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-1 h-1 bg-[#8B7FFF] rounded-full" />
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} className="w-1 h-1 bg-[#8B7FFF] rounded-full" />
                    </div>
                  </div>
                ) : isCompanionOnline ? (
            ...
            <button onClick={() => setIsSelectionMode(true)} className="p-2 rounded-xl text-[#A0AEC0] hover:text-red-400 hover:bg-red-400/10 transition-colors">
      ...
      >
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              {isSelectionMode && (
                <div className={`absolute inset-0 -mx-4 px-4 z-0 transition-colors ${isSelected ? "bg-[#8B7FFF]/10" : "hover:bg-white/5"}`} />
              )}

              <div className={`relative z-10 flex items-end gap-2 max-w-[85%] sm:max-w-[70%] ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                {isSelectionMode && (
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mb-1 transition-all ${isSelected ? "bg-[#8B7FFF] border-[#8B7FFF] scale-110" : "border-[#A0AEC0]/60"}`}>
                    {isSelected && <Check className="w-3 h-3 text-white" />}
                  </div>
                )}

                <div className={`px-5 py-3 rounded-2xl text-sm sm:text-base shadow-sm transition-transform ${isSelectionMode && isSelected ? "scale-[0.98]" : ""} ${isMine ? "bg-linear-to-br from-[#8B7FFF] to-[#6DD5ED] text-white rounded-tr-sm" : "bg-[#1A1F2E] text-[#e2e8f0] border border-white/5 rounded-tl-sm"} ${isDeletedForEveryone ? "opacity-80" : ""}`}>
                  {isDeletedByMe ? (
                    <p className="flex items-center gap-2 italic text-white/70 text-xs sm:text-sm">
                      <Ban className="h-3 w-3 shrink-0" /> <span>Вы удалили данное сообщение</span>
                    </p>
                  ) : isDeletedByThem ? (
                    <p className="flex items-center gap-2 italic text-white/70 text-xs sm:text-sm">
                      <Ban className="h-3 w-3 shrink-0" /> <span>Данное сообщение удалено</span>
                    </p>
                  ) : (
                    <p className="break-words whitespace-pre-wrap">{msg.text}</p>
                  )}
                  
                  <div className={`flex items-center gap-1 mt-1 text-[10px] ${isMine ? "justify-end text-white/80" : "justify-start text-[#A0AEC0]"}`}>
                    <span>{time}</span>
                    {!isDeletedForEveryone && isMine && (
                      msg.is_read ? <CheckCheck className="w-3.5 h-3.5 text-white" /> : <Check className="w-3 h-3 text-white/70" />
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="p-4 sm:p-6 bg-[#1A1F2E] border-t border-white/5 relative overflow-hidden">
        <AnimatePresence mode="wait">
          {isSelectionMode ? (
            <motion.div key="delete-panel" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="flex items-center justify-between">
              <button onClick={cancelSelection} className="px-4 py-3 rounded-xl text-white hover:bg-white/5 font-medium transition-colors">Отмена</button>
            ...
            <motion.form key="input-panel" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} onSubmit={handleSend} className="flex items-end gap-3">
              ...
              <button type="submit" disabled={!inputValue.trim()} className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#8B7FFF] flex items-center justify-center text-white shrink-0 hover:bg-[#7a6ee6] active:scale-95 transition-all disabled:opacity-50">
