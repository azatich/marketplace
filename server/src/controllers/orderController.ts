    ...
    static async createOrder(req: Request, res: Response) {
        try {
            ...
            if (orderError || !orderData) {
                return res.status(500).json({ message: "Ошибка при создании заказа", error: orderError?.message });
            }
            ...
            if (productsError || !productsData) {
                ...
                return res.status(500).json({ message: "Ошибка при получении данных о товарах", error: productsError?.message });
            }
            ...
            if (itemsError) {
                ...
                await supabase.from('orders').delete().eq('id', orderData.id);
                return res.status(500).json({ message: "Ошибка при сохранении товаров заказа", error: itemsError.message });
            }
